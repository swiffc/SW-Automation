...
namespace Plenum.Floor.Derived.Derived
{
    internal class OuterFloorPanel : FloorPanel
    {
        ...
        public static bool Enabled
        {
            get
            {
                return CallerType == CallerType.Legacy ? false : true;
            }
        }
        ...
        // Constructor
        public OuterFloorPanel(CallerType callerType) : base(callerType) { }


        internal static void GetLengthAndRotation(out double zColumnnCut, out double rotate)
        {
            rotate = 0;
            if (CallerType == CallerType.Johnson && FanCount != 1)
            {
        ...
        protected override void FeatureSuppression(ModelDoc2 modelDoc2)
        {
            ...
            if (CallerType == CallerType.Johnson && zColumnCut + Beam.FlangeWidth/2 < nominalLength)
        ...
        internal static double GetNominalLength(CallerType callerType)
        {
            ...
            switch (callerType)
            {
                case CallerType.Standard:
                    calculatedLength = Length / (FanCount * 2) + Beam.Depth / 2 - bTools.GetBendRadius(EndPanel.THK) - mTools.AssemblyClearance / 2;
                    break;
                case CallerType.Johnson:
                    if (FanCount == 1)
                    {
                        calculatedLength = Length / FanCount / 2 + Johnson.ExtraLength - bTools.GetBendRadius(EndPanel.THK) - mTools.AssemblyClearance / 2;
                        break;
                    }
                    calculatedLength = (Length / FanCount + Johnson.ExtraLength) / 2 - bTools.GetBendRadius(EndPanel.THK);
                    break;
                case CallerType.Legacy:
                    calculatedLength = Length / (FanCount * 2) - bTools.GetBendRadius(EndPanel.THK);
                    break;
            }
